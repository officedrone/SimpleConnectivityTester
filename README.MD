# Simple Python Network Connectivity Tester with UI
...
1. Clone the repo and navigate to the folder in which the app is saved
2. Run `python connectivity-tester.py` 
3. See bottom of this file for environment prep and other instructions if needed
...
- **Manual testing**: Test to a single IP:port combinations directly from the main form, or perform DNS lookup (lookup done against the portion of the string before the column ':' symbol)
...
- **IP Refresh Functionalty**: Refresh IP addresses you can test from without having to restart the app. Useful if you change VPNs or VPN gateways

- **Configurable test delay**: Configure delay between each tests (default is 100ms).
...
2. **Run the Application**
   - Execute the Python script by running `python connectivity-tester.py` to launch the main window

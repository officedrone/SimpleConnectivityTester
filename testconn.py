...
def run_task_async(tasks, tree, index, button_name, source_ip=None):
    ...
    def scan_worker():
        ...
        def update_ui():
            ...
            tree.item(
                iid,
                values=(current[0], current[1], current[2], status),
                tags=(tag,)
            )
            tree.see(iid)
...
def create_main_window():
    ...
    def _run_manual():
        ...
        try:
            ...
            result_text = "SUCESSFUL" if success else "UNSUCCESFUL"

        # Update the read‑only entry with color
        result_var.set(result_text)
        color = "#27ae60" if success else "#c0392b"
        result_entry.configure(foreground=color)   # change text colour
